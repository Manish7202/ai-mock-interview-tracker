document.addEventListener("DOMContentLoaded", function () {
    ...
    async function generateScorecard() {
        ...
        if (micStream && micStream !== webcamStream) {
    ...
    async function initializeMedia() {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            console.warn("Media devices or getUserMedia is not supported in this browser.");
            const pip = document.getElementById("webcam-pip");
            if (pip) pip.style.display = "none";
            return;
        }

        const videoRequested = camToggle.checked;
        const audioRequested = micToggle.checked;

        if (videoRequested && audioRequested) {
            try {
                // Request both together to present a single unified permission prompt
                ...
                console.warn("Combined media request failed, trying separately:", err);
                await initializeMediaSeparately(videoRequested, audioRequested);
            }
        } else {
            await initializeMediaSeparately(videoRequested, audioRequested);
        }
    ...
    async function initializeMediaSeparately(videoRequested, audioRequested) {
        if (videoRequested) {
            try {
